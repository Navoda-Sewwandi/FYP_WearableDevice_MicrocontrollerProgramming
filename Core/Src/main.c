...
static uint8_t Tdata= 5;
int connected =6;

uint8_t my_value[10] = {9,2,3,1,2,3,4,5};
uint8_t val_read =0;

//variables for EDA reading
uint16_t eda_data = 0;
uint16_t sum = 0;
uint16_t gsr_signal = 0;
uint16_t EDA_transmit =0;
uint16_t eda_arr[1000];
uint8_t reg_addr =0x4F;
uint8_t reg_addr1= 0x46;
int value1;
int read_values[50];
int16_t value[75];
int i=0;
...
uint16_t ppg_signal = 0;
uint16_t PPG_transmit=0;
uint16_t PPG_arr[100];
int b = 0; // for taking ppg data
  ...
  while (1)
  {
	  ...
	  if(val_read==1){
		  for(a = 0; a<720; a++){
			  ...
			  eda_arr[a] = gsr_signal;
			  all_data[1] = gsr_signal;
...
void ReadPPG(){
	...
	// Read ADC value
	ppg_signal= HAL_ADC_GetValue(&hadc1);
